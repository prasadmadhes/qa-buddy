...
- **Notes use TEACHING-FLOW style, not reference style.** Open with the puzzle, build narratively (puzzle → why obvious answer fails → right approach → details → quick check), use conversational "we"/"let's"/"notice" tone, include "quick check before moving on" beats between sections. Group reference material (tables, ref cards, mistakes) into a clearly-labeled `## Reference appendix` at the BOTTOM. Don't open with a 30-second mental model — let the lesson build to it. The user re-reads to RE-LEARN, not to look up a fact. (Full format spec: `feedback_teaching_flow_notes.md` in memory.)
- **Doubt-driven inline callouts (layered on teaching-flow).** Embed the user's actual session doubts as inline 🤔 callouts at the natural confusion point in the narrative, PLUS a `### Doubts that came up — quick index` section in the reference appendix. (Full format spec: `feedback_doubt_driven_notes.md` in memory.)
- **Use Obsidian/GitHub callout syntax** for all callouts in notes — `> [!note]`, `> [!tip]`, `> [!warning]`, `> [!question]`, `> [!example]`, `> [!summary]`, `> [!important]`, `> [!info]`, `> [!abstract]`. Render as colored boxes with icons in Obsidian (the user's reader). Don't use plain blockquotes with emoji prefixes (`> 💡 ...`) — they all render identically grey, defeating the visual hierarchy. Mapping: doubts → `[!question]`, quick-asides → `[!tip]`, warnings → `[!warning]`, quick-check sections → `[!example]`, one-line takeaway → `[!important]`, TL;DR → `[!summary]`, opening lesson-pitch → `[!abstract]`. (Full format spec: `feedback_obsidian_callouts.md` in memory.)
